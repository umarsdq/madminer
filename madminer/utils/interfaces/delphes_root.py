...
def _get_particles_leptons(tree, pt_min_e, eta_max_e, pt_min_mu, eta_max_mu):
    ...
    for ievent in range(len(pt_mu)):
        ...
        # Combined muons and electrons (convert awkward arrays to numpy)
        event_pts = np.concatenate((np.array(pt_mu[ievent]), np.array(pt_e[ievent])))
        event_etas = np.concatenate((np.array(eta_mu[ievent]), np.array(eta_e[ievent])))
        event_phis = np.concatenate((np.array(phi_mu[ievent]), np.array(phi_e[ievent])))
        event_masses = np.concatenate((0.105 * np.ones_like(np.array(pt_mu[ievent])), 0.000511 * np.ones_like(np.array(pt_e[ievent]))))
        event_charges = np.concatenate((np.array(charge_mu[ievent]), np.array(charge_e[ievent])))
        event_pdgid_positive_charges = np.concatenate(
            (-13 * np.ones_like(np.array(pt_mu[ievent]), dtype=int), -11 * np.ones_like(np.array(pt_e[ievent]), dtype=int))
        )
